...
class GaussianDiffusion1D(nn.Module):
    ...
    def sample(self, label, age, datatype, get_warmup=False, batch_size = 65):
        ...
        for v in range(label.shape[1]):
            ...
            sampled = sample_fn(init_data, label[:, v, :], age[:, v], datatype[:, v], get_warmup, (batch_size, channels, num_node))
                ...
                if accelerator.is_main_process:
                    ...
                    if (self.step != 0 and self.step % self.save_and_sample_every == 0) or (self.step == self.train_num_steps - 1):
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(map(lambda n: self.ema.sample(label=test_label, age=test_age, datatype=test_datatype, batch_size=self.test_size), batches))
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(map(lambda n: self.ema.sample(label=train_label, age=train_age, datatype=train_datatype, batch_size=162), batches))
