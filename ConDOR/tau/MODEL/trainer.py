...
class warmup_Dataset1D(Dataset):
    ...
    def collate_fn(self, samples):
        ...
        X, Y, A, M, D = torch.stack(collate_x), torch.stack(collate_y), torch.stack(collate_a), torch.stack(collate_mask), torch.stack(collate_type)
        return X, Y, A, M, D
...
class GaussianDiffusion1D(nn.Module):
    ...
    def model_predictions(self, x, label, age, datatype, t, get_warmup = False, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
        if get_warmup == True:
    ...
    def ddim_sample_init(self, init_data, label, age, datatype, get_warmup, shape, clip_denoised = True):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(data, label, age, datatype, time_cond, get_warmup, self_cond, clip_x_start = clip_denoised)
    ...
    def ddim_sample_fromv2(self, init_data, label, age, datatype, shape, clip_denoised = True):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(data, label, age, datatype, time_cond, self_cond, clip_x_start = clip_denoised)
            data += pred_noise 
    ...
    def p_losses(self, x_start, label, age, datatype, visit, t, x_diff=None, noise=None): 
        if visit == 0: # first visit
            ...
            cohort_x_t_plus_1 = self.q_sample_between_visits(label, age, datatype, t+1) # x_(t+1) is sampled from P(X|Y=y, A=a)
    ...
    def forward(self, data, label, age, datatype, *args, **kwargs):
        ...
        if v == 1:
            ...
            model_out, noise = self.p_losses(data[:, :, 0, :], label[:, 0, :], age[:, 0], datatype[:, 0], 0, t, *args, **kwargs)
            return model_out, noise
        else: 
            ...
            for i in range(1, v):
                ...
                model_out, noise = self.p_losses(data[:, :, i-1:i+1, :], label[:, i-1:i+1, :], age[:, i-1:i+1], datatype[:, i], i+1, t, x_diff, *args, **kwargs)
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        train_dl = DataLoader(train_dataset, batch_size=162, collate_fn=train_dataset.collate_fn, shuffle = False, pin_memory = True, num_workers = cpu_count())
        ...
        with tqdm(initial = self.step, total = self.warmup_num_steps, disable = not accelerator.is_main_process) as pbar:
            while self.step < self.warmup_num_steps:
                ...
                with torch.autograd.detect_anomaly():
                    for _ in range(self.gradient_accumulate_every):
                        ...
                        with self.accelerator.autocast():
                            model_out, noise = self.model(x[:, :, :].unsqueeze(-2), label[:, :].unsqueeze(-2), age[:].unsqueeze(-1), datatype[:].unsqueeze(-1))
                ...
                if accelerator.is_main_process:
                    ...
                    if (self.step != 0 and self.step % self.save_and_sample_every == 0) or (self.step == self.train_num_steps - 1) or (self.step >= 7000 and self.step % 10 == 0):
                        self.ema.eval()
                        ...
                        with torch.no_grad():
                            ...
                            test_x, test_label, test_age, test_mask, test_dt = test_data[0].cuda(), test_data[1].cuda(), test_data[2].cuda(), test_data[3].cuda(), test_data[4].cuda()
                            ...
                            all_samples_list = list(map(lambda n: self.ema.sample(label=test_label, age=test_age, datatype=test_dt, batch_size=self.test_size), batches))
        ...
        if self.warmup == True:
            ...
            with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
                while self.step < self.train_num_steps:
                    ...
                    if accelerator.is_main_process:
                        self.ema.train()
                        if (self.step > 1000 and self.step % self.save_and_sample_every == 0) or (self.step == self.train_num_steps - 1) or (self.step >= 7000 and self.step % 10 == 0):
                            self.ema.eval()
                            print('Sampling in Epoch #', self.step)
                            ...
                            with torch.no_grad():
                                ...
                                test_x, test_label, test_age, test_mask, test_datatype = test_data[0].cuda(), test_data[1].cuda(), test_data[2].cuda(), test_data[3].cuda(), test_data[4].cuda()
                                ...
                                if milestone == 1:
                                    ...
                                    image = wandb.Image(test_x[0].unsqueeze(0), caption=fn, file_type="png") ## visualize first data ##########
                                    wandb.run.log({"test sample": [image]})
                                all_samples_list = list(map(lambda n: self.ema.sample_fromv2(label=test_label, age=test_age, datatype=test_datatype, batch_size=self.test_size), batches))
                            ...
                            with torch.no_grad():
                                milestone = self.step // self.save_and_sample_every
                                batches = num_to_groups(self.num_samples, 162)

                                train_data = next(self.train_dl)
                                train_x, train_label, train_age, train_mask, train_datatype = train_data[0].cuda(), train_data[1].cuda(), train_data[2].cuda(), train_data[3].cuda(), train_data[4].cuda()
                                train_datatype = train_datatype.unsqueeze(-1)
                                all_samples_list = list(map(lambda n: self.ema.sample_fromv2(label=train_label, age=train_age, datatype=train_datatype, batch_size=162), batches))
                            ...
                            if rmse < rmse_min:
                                ...
                                if wd < 10:
                                    fn = 'best_model-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(round(js, 3)) + '_' + str(round(rmse, 3)) + '_' + str(round(train_wd, 3)) + '_' + str(round(train_js, 3)) + '_' + str(round(train_rmse, 3)) + '.pt'
                                    pth = os.path.join(self.results_folder, fn)
                                    self.save(pth)
