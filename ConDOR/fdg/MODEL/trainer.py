        ...
        if self.warmup == True:
            ...
            with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
                while self.step < self.train_num_steps:
                    ...
                    self.scheduler.step()
                    if accelerator.is_main_process:
                        if (self.step > 1000 and self.step % self.save_and_sample_every == 0) or (self.step == self.train_num_steps - 1) or (self.step >= 8000 and self.step % 10 == 0):
                            self.ema.eval()
                            print('Sampling in Epoch #', self.step)
                            ...
                            if wd is not None and js is not None and rmse is not None:
