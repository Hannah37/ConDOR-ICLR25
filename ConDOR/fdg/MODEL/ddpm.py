...
class Unet1D(nn.Module):
    def __init__(
        ...
        attn_heads = 4,
        n_classes = None,
        max_visit = None #TODO: maybe delete?
    ):
        super().__init__()
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            # print(ind, dim_in, dim_out) # 0 64 64, 1 64 128, 2 128 256, 3 256 512
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        train_dl = DataLoader(train_dataset, batch_size=542, collate_fn=train_dataset.collate_fn, shuffle = False, pin_memory = True, num_workers = cpu_count())
                ...
                if accelerator.is_main_process:
                    if (self.step != 0 and self.step % self.save_and_sample_every == 0) or (self.step == self.train_num_steps - 1):
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(map(lambda n: self.ema.sample(label=train_label, age=train_age, batch_size=542), batches))
                        all_samples = torch.cat(all_samples_list, dim=0)
