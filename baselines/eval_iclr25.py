...
def evaluate(real_data_path, gen_data_path, SPLIT = 'train', dn='Amyloid'):
    if 'tab_ddpm' in gen_data_path:
        x_gen = np.load(os.path.join(gen_data_path, f'X_num_norm_{SPLIT}.npy'), allow_pickle=True)#[:, :-1]
    else:
...
for split in ['val', 'train']:
    for dn in ['Amyloid', 'CT', 'FDG', 'Tau']:
        ...
        for i in range(1, 4):
            ...
            parent_dir = f'exp/{dn}/{methodn}{i}'
            config = f'{parent_dir}/config.toml'
            if 'tab_ddpm' == methodn:
                if not os.path.exists(os.path.join(parent_dir, f'X_num_norm_{split}.npy')): continue
            elif 'ctgan' == methodn:
                if not os.path.exists(f'CTGAN/generate_sample_ctgan{i}/{dn}/X_num_{split}.npy'): continue
                parent_dir = f'CTGAN/generate_sample_ctgan{i}/{dn}'
            else:
                if not os.path.exists(os.path.join(parent_dir, f'X_num_{split}.npy')): continue
